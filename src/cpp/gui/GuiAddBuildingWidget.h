#ifndef _GUI_ADD_BUILDING_WIDGET_H
#define _GUI_ADD_BUILDING_WIDGET_H

#include <SDL_render.h>
#include "gui/GuiBase.h"

union SDL_Event;
class Graphic;
class SDL_Renderer;


/**
* GUI-Widget, was das Gebäude anzeigt, was aktuell platziert wird. Zusätzlich enthält es die Controls zum Drehen (TODO),
* die Baukosten und die Infos, was es produziert.
*/
class GuiAddBuildingWidget : public GuiBase {

public:
    GuiAddBuildingWidget();
    virtual ~GuiAddBuildingWidget() {}

    /**
    * @brief Zeichnet das Element (ohne Kinder)
    */
    virtual void renderElement(SDL_Renderer* renderer);

    /**
    * @brief Callback, der ein Event handelt
    * @param event SDL-Event
    */
    virtual void onEventElement(SDL_Event& event) {
        // nix tun
    }

};

#endif
